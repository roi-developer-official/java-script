...
function bubbleSort(array) {
    ...
    let i = 0;
    bubbleSortInterval = setInterval(() => {
        if (i === j1.length) {
...
function switchNodes(i, j, items) {
    ...
    items[j].style.background = 'rgba(20,202,10)';
    setTimeout(() => {
...
function merge(left, right) {
    ...
    for(let i = 0; i < resultArray.length; i++){
        rangeAraay.push(resultArray[i].id);
    }
   
    replaceNodes(rangeAraay);
    return resultArray;
}
...
function replaceNodes(range){
    ...
    let i = min;
        let interval = setInterval(()=>{
            ...
            items[i].parentElement.replaceChild(resultArray[j].cloneNode(true), items[i]);
